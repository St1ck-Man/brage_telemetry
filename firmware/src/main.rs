...
use embassy_stm32::peripherals::{FDCAN1, USB_OTG_FS};
...
async fn main(spawner: embassy_executor::Spawner) {
    ...
    let _usb_driver = embassy_stm32::usb::Driver::new_fs(
        p.USB_OTG_FS,
        Irqs,
        p.PA12,
        p.PA11,
        unsafe { USB_BUF.get_mut() },
        config,
    );
    ...
    let mut can_configurator =
        embassy_stm32::can::CanConfigurator::new(p.FDCAN1, p.PB8, p.PB9, Irqs);
    ...
    static mut CAN_TX_BUF: core::cell::UnsafeCell<embassy_stm32::can::TxFdBuf<1024>> =
        core::cell::UnsafeCell::new(embassy_stm32::can::TxFdBuf::new());
    static mut CAN_RX_BUF: core::cell::UnsafeCell<embassy_stm32::can::RxFdBuf<1024>> =
        core::cell::UnsafeCell::new(embassy_stm32::can::RxFdBuf::new());

    let can = can.buffered_fd(unsafe { CAN_TX_BUF.get_mut() }, unsafe {
        CAN_RX_BUF.get_mut()
    });
    ...
    spawner
        .spawn(radio::radio_task(
            spi, sx_cs, sx_busy, sx_reset, sx_dio1, sx_dio2, sx_dio3, led_tx, led_rx, pa_en,
            pa_tx_en, pa_rx_en, vref_en,
        ))
        .unwrap();
